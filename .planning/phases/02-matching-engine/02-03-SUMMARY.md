...
plan: 03
subsystem: websocket, notifications, frontend
tags: [websocket, redis-pubsub, toast, progress-tracker, real-time]

# Dependency graph
requires:
  - phase: 02-matching-engine/02
    provides: "full Celery matching task with publish_notification on completion/failure"
provides:
...
affects: [03-review-merge]

# Tech tracking
tech-stack:
  added: [websockets]
  patterns:
    - "Redis pub/sub async subscription in WebSocket handler via redis.asyncio"
    - "Lazy singleton Redis client for sync publishing from Celery workers"
    - "WebSocket URL derived from window.location.host — Vite proxy handles /ws in dev"
    - "Exponential backoff reconnection (1s → 2s → 4s → ... → 30s max)"
    - "Heartbeat ping/pong every 30s with 10s timeout for connection liveness"
