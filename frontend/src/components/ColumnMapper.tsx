...
// Dark Precision Editorial — polished step indicators, atmospheric form, styled mapping rows
    ...
    <form onSubmit={handleSubmit} className="card overflow-hidden">
      ...
      <div className="px-6 pt-6 pb-5 border-b border-white/[0.06]">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2.5">
            <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-accent-500/15 border border-accent-500/30 text-xs font-bold text-accent-300 shadow-sm shadow-accent-500/10">
              1
            </div>
            <div>
              <p className="text-xs font-semibold text-accent-300 uppercase tracking-wider">Define</p>
              <p className="text-[10px] text-accent-500/60">Name & describe</p>
            </div>
          </div>

          {/* Connecting line with gradient fill */}
          <div className="flex-1 h-[2px] rounded-full bg-surface-700/40 overflow-hidden">
            <div
              className="h-full rounded-full bg-gradient-to-r from-accent-500/60 to-accent-400/40 transition-all duration-700 ease-out"
          ...
          <div className="flex items-center gap-2.5">
            <div className={`flex items-center justify-center w-8 h-8 rounded-lg border text-xs font-bold transition-all duration-500 ${
              sourceName.trim()
                ? 'bg-accent-500/15 border-accent-500/30 text-accent-300 shadow-sm shadow-accent-500/10'
            ...
            <div>
              <p className={`text-xs font-semibold uppercase tracking-wider transition-colors duration-300 ${
                sourceName.trim() ? 'text-accent-300' : 'text-surface-600'
              }`}>Map</p>
              <p className={`text-[10px] transition-colors duration-300 ${
                sourceName.trim() ? 'text-accent-500/60' : 'text-surface-700'
              }`}>CSV columns</p>
            </div>
          </div>
        </div>
      ...
      <div className="p-6 border-b border-white/[0.06]">
        <div className="flex items-center gap-3 mb-5">
          ...
          <h3 className="text-sm font-display text-white tracking-wide">Name your source</h3>
        </div>
          ...
          <div>
            ...
            <input
              ...
              className={`input-field w-full ${
                errors.sourceName
                  ? '!border-danger-500/40 focus:!border-danger-400 focus:!ring-danger-500/10'
                  : ''
              }`}
            />
            {errors.sourceName && (
      ...
      <div className="p-6 border-b border-white/[0.06]">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-sm font-display text-white tracking-wide">Map columns</h3>
          </div>
          ...
          <div className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-md transition-colors ${
            requiredMapped === requiredTotal
              ...
              : 'bg-surface-800/40 text-surface-500 border border-white/[0.04]'
          }`}>
            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="grid grid-cols-[1fr,auto,1fr] gap-3 mb-3 px-3">
          <p className="text-[11px] font-semibold text-accent-400/70 uppercase tracking-[0.12em]">Canonical Field</p>
          <div className="w-6" />
          <p className="text-[11px] font-semibold text-accent-400/70 uppercase tracking-[0.12em]">CSV Column</p>
        </div>
            ...
            return (
              <div
                ...
                className={`
                  ...
                  animate-slideUp
                  ${hasError
                    ? 'bg-danger-500/[0.04] border border-danger-500/10'
                    : isMapped
                      ? 'bg-success-500/[0.03] border border-success-500/[0.06] hover:bg-success-500/[0.05]'
                      : 'bg-surface-800/20 border border-transparent hover:bg-surface-800/40 hover:border-white/[0.04]'
                  }
                `}
              ...
              >
                ...
                <div className="flex items-center gap-2.5">
                  <div className={`w-2 h-2 rounded-full shrink-0 transition-all duration-300 ${
                    isMapped
                      ? 'bg-success-400 shadow-sm shadow-success-400/30'
                      : field.required
                        ? 'bg-danger-400/60 animate-pulse'
                        : 'bg-surface-600'
                  }`} />
                  <span className={`text-sm transition-colors ${
                    ...
                    {field.label}
                    {field.required && <span className="text-danger-400 ml-0.5 text-xs">*</span>}
                  </span>
                  {field.required && (
                    <span className="text-[9px] font-semibold uppercase tracking-wider text-accent-500/50 bg-accent-500/[0.06] px-1.5 py-0.5 rounded">
                      req
                    </span>
                  )}
                </div>
                ...
                <div className={`flex items-center justify-center w-6 transition-colors duration-300 ${
                  isMapped ? 'text-success-500/60' : 'text-surface-700'
                }`}>
                  ...
                  className={`w-full rounded-lg border px-3 py-2 text-sm outline-none transition-all bg-surface-800/50 appearance-none cursor-pointer ${
                    hasError
                      ...
                      : isMapped
                        ...
                        : 'border-white/[0.08] text-surface-500 hover:border-white/[0.14]'
        ...
        {(errors.supplier_name || errors.supplier_code) && (
          <div className="mt-4 flex items-start gap-2.5 rounded-xl border border-danger-500/20 bg-danger-500/[0.06] px-4 py-3 animate-slideUp">
            <svg className="w-4 h-4 text-danger-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
            </svg>
            <div className="text-xs text-danger-400 space-y-0.5">
        ...
        <div className="mt-5 flex items-center gap-3 text-xs text-surface-500">
            ...
            <div
              className={`h-full rounded-full transition-all duration-500 ease-out ${
                mappedCount === totalFields
                  ? 'bg-gradient-to-r from-success-500/80 to-success-400/60'
                  : 'bg-gradient-to-r from-accent-500/60 to-accent-400/40'
              }`}
              style={{ width: `${(mappedCount / totalFields) * 100}%` }}
            />
          </div>
          {mappedCount === totalFields && (
            <span className="text-success-400 flex items-center gap-1 animate-fadeIn">
              <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {/* ── Submit footer ── */}
      <div className="flex items-center justify-between gap-3 p-6 bg-surface-900/40">
